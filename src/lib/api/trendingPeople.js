...
export const getTrendingPeople = async (loadMore = false) => {
	...
	let currentPage = trendingPageNumber;
	...
	for (let i = 0; i < 3; i++) {
		const options = {
			method: 'GET',
			url: `https://api.themoviedb.org/3/trending/person/week?language=en-US&page=${currentPage}`,
			headers: {
		...
		try {
			...
			if (currentPage >= response.data.total_pages) break; // Compare with the response data directly
		} catch (error) {
			console.error(error);
		}
	}
